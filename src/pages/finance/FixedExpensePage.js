(function() {
/**
 * Personal Hub — 固定支出管理页面
 * 管理每月固定支出记录（不算入收支结余，仅显示）
 */
...
function FixedExpensePage() {
  ...
  const handleDelete = async (record) => {
    await DAO.fixedExpense.delete(record.id);
    Haptics.warning();
  ...
  const renderRecord = (record) => {
    ...
    return h('div', {
      ...
      h('span', {
        style: { fontSize: '17px', fontWeight: 600, color: 'var(--color-deadline)', flexShrink: 0 },
        className: 'numeric'
  ...
  return h('div', { style: { display: 'flex', flexDirection: 'column', height: '100%' } },
    h(NavBar, { title: '固定支出', showBack: true }),
    ...
    h('div', { className: 'scroll-container page-with-action-bar' },
      ...
      h(Card, { style: { marginBottom: 'var(--space-md)' } },
        h('div', { style: { textAlign: 'center', padding: 'var(--space-sm) 0' } },
          h('div', { style: { fontSize: '13px', color: 'var(--color-text-tertiary)' } }, `${DateUtils.monthLabel(financeMonth)} 固定支出`),
          h('div', { style: { fontSize: '28px', fontWeight: 700, color: 'var(--color-deadline)' }, className: 'numeric' }, FormatUtils.money(monthlyTotal)),
        ),
      ),
      ...
      }, '固定支出仅显示在生活记账的概览中，不计入收支结余'),

      // 记录列表
      records.length === 0
        ? h(EmptyState, { icon: '💸', title: '还没有固定支出记录', subtitle: '点击下方按钮添加' })
    ...
    h(FixedExpenseForm, { open: showForm, onClose: handleCloseForm, record: editingRecord })
  );
}

// ===== 固定支出编辑表单 =====

function FixedExpenseForm({ open, onClose, record }) {
  ...
  const handleSave = async () => {
    ...
    if (record) {
      await DAO.fixedExpense.update(record.id, data);
      showToast('已更新', 'success');
    } else {
      await DAO.fixedExpense.create(data);
  ...
  return h(Sheet, { open, onClose, title: record ? '编辑固定支出' : '新增固定支出' },
...
window.FixedExpensePage = FixedExpensePage;
