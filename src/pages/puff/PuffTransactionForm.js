...
function PuffTransactionForm({ open, onClose, type: initialType, transaction }) {
  const { showToast } = useApp();
  const [type, setType] = useState('expense');
  const [amount, setAmount] = useState('');
  const [category, setCategory] = useState('puff_ingredient');
  ...
  useEffect(() => {
    if (open) {
      if (transaction) {
        ...
        setAmount('');
        setCategory(initialType === 'income' ? 'puff_sales' : 'puff_ingredient');
  ...
  const handleTypeChange = (newType) => {
    setType(newType);
    setCategory(newType === 'income' ? 'puff_sales' : 'puff_ingredient');
    Haptics.selection();
  };
  ...
  const cats = type === 'income' ? CATEGORIES.puffIncome : CATEGORIES.puffExpense;
  ...
  return h(Sheet, { open, onClose, title: transaction ? '编辑泡芙记录' : '新增泡芙记录' },
...
window.PuffTransactionForm = PuffTransactionForm;
